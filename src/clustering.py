...
def clustering(DIJKSTRA: Pathfinder, nodes, n):
	...
	while len(clusters) < n:  # As long as the right number of clusters has not been constituted
		...
		for i in range(n - n_clusters):
			...
			print("Deleting the edge between ", highest_betweenness[0], " and ", highest_betweenness[1], " (Betweenness: ", edge_betweenness[highest_betweenness], ")", sep='')
	...
	print("\n", n, "clusters obtained:")
	for i in range(len(clusters)):
		print("Cluster #", i + 1, " found, size: ", len(clusters[i]), " (", round(100 * len(clusters[i]) / n_nodes), "%) : ", sorted(clusters[i]), sep='')
